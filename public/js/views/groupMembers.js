...
import { iconChevronLeft, iconChevronRight, iconPlus, iconClose } from '../icons.js';
  ...
  var rows = members.map(function (mm, idx) {
    ...
    return '<div class="list-row" data-action="open-member-payments" data-gid="' + gid + '" data-mid="' + mm.id + '" style="cursor:pointer;">' +
      ...
      '<div style="flex:1 1 auto; min-width:0;">' +
        ...
        (win ? '<div style="font-size:11px;color:#146b52;margin-top:1px;">Received ' + fmt(win.amount) + ' · ' + monthLabel(group.startYear, group.startMonthIndex, win.month) + '</div>' : '') +
      '</div>' +
      (canRemove ? '<div data-action="remove-member-from-group" data-gid="' + gid + '" data-mid="' + mm.id + '" style="cursor:pointer;color:var(--danger);font-size:12px;font-weight:600;">Remove</div>' : '') +
      iconChevronRight() +
    '</div>';
  ...
  var html = '' +
    '<div class="screen">' +
      '<div class="topbar">' +
        ...
        '<div><div class="title">Members</div><div class="subtitle">' + escapeHtml(group.name) + '</div></div>' +
      '</div>' +
      '<div class="content">' +
