...
import { iconPlus, iconClose } from '../icons.js';
...
export function renderMembers() {
  ...
  var rows = allMembers.map(function (mm, idx) {
    ...
    return '<div class="list-row" ' + (isSuper() ? '' : 'data-action="open-member-form" data-id="' + mm.id + '"') + '>' +
      ...
      '<div style="font-size:11.5px;color:var(--text-muted);margin-top:1px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + escapeHtml(subtitle) + '</div></div>' +
    '</div>';
  }).join('') || '<div class="card" style="color:var(--text-muted);font-size:13px;text-align:center;">No members yet — tap + to add one.</div>';

  var html = '' +
    '<div class="screen">' +
      '<div style="padding:20px 20px 4px;"><div class="mono" style="font-size:20px;font-weight:700;">Members</div>' +
...
function renderMemberFormOverlay(mf) {
  ...
  return '<div class="overlay"><div class="sheet">' +
    '<div class="sheet-header"><div style="font-size:14px;font-weight:700;">' + (isNew ? 'Add member' : 'Edit member') + '</div>' +
