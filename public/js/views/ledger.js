...
export function renderLedger() {
  ...
  var filterChips = FILTERS.map(function (f) {
    ...
    return '<div data-action="set-ledger-filter" data-filter="' + f.key + '" style="display:flex;align-items:center;gap:5px;cursor:pointer;flex-shrink:0;padding:8px 14px;border-radius:20px;font-size:12.5px;font-weight:600;' +
      (active ? 'background:var(--accent);color:#fff;' : 'background:#fff;color:var(--text);border:1px solid var(--border);') + '">' + f.icon(color) + f.label + '</div>';
  }).join('');
    ...
    '<div class="screen">' +
      ...
      '<div class="content">' +
        ...
        '<div class="card" style="display:flex;justify-content:space-between;align-items:center;">' +
          '<div style="display:flex;align-items:center;gap:5px;font-size:13px;color:var(--text-muted);">' + iconWallet() + 'Total in ledger</div><div class="mono" style="font-size:15px;font-weight:700;">' + fmt(state.balances.total) + '</div>' +
        '</div>' +
        '<div class="banner info"><div class="banner-title">' + iconInfo('var(--accent)') + 'How transfers work</div><div style="font-size:12px;color:var(--accent);line-height:1.4;">There\'s no direct admin-to-admin transfer here. Funds only move between admins per month, tied to what was actually collected, and need the other admin\'s acceptance — open a month to request one.</div></div>' +
