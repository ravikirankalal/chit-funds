      ...
      '<div class="content">' +
        ...
        (function () {
          ...
          return '<div class="card" style="display:flex;flex-direction:column;gap:8px;">' +
            ...
            '<div style="display:flex;justify-content:space-between;"><div style="display:flex;align-items:center;gap:4px;font-size:12px;color:var(--text-muted);">' + iconTrendingUp() + 'Profit margin</div><div style="font-size:13px;font-weight:700;color:' + marginColor + ';">' + marginFormatted + ' (' + pct.toFixed(1) + '%)</div></div>' +
          '</div>';
        })() +
        '<div style="height:1px;background:var(--border);"></div>' +
        '<div>' +
...
function renderStep2(g) {
  ...
  var memberRows = g.members.map(function (m, idx) {
    return '<div class="list-row" style="cursor:default;"><div class="avatar sm" style="background:' + colorFor(idx) + ';">' + initialsOf(m.name) + '</div>' +
      ...
      '<div data-action="remove-draft-member" data-idx="' + idx + '" style="display:flex;align-items:center;gap:4px;cursor:pointer; color:var(--danger); font-size:12px; font-weight:600;">' + iconTrash('var(--danger)') + 'Remove</div></div>';
  }).join('');
